function out = model_SV(y, Z, splitIdx, opts)
% MODEL_SV  Self-contained standard stochastic-volatility (SV) model.
% =====================================================================
% ONE FILE, NO DEPENDENCIES. Fits the plain Gaussian "standard SV" model
% via the SMC particle-filter engine, produces out-of-sample variance
...
% USAGE
% -----
%   model_SV                          % built-in synthetic demo
%   out = model_SV(y)                 % your T-by-1 returns; 80/20 split
%   out = model_SV(y, Z)              % Z is ignored by SV (variance has no covariates)
%   out = model_SV(y, Z, splitIdx)
%   out = model_SV(y, Z, splitIdx, opts)
%
...
% flattening for the SV model only). Statistics & ML Toolbox is used for
...
% =====================================================================
    ...
    if nargin < 1 || isempty(y)
        ...
        fprintf('[model_SV] No data supplied -> built-in synthetic demo.\n');
    else
        if nargin < 2; Z = []; end
